...
def FindAngles(T): 
    """
    Find the Euler angles. Outputs [[],[theta(1,2)],...,[theta(1,N),...,theta(N-1,N)]]
    """
    N = T.shape[0]
    anglesList = []    
    TNu=T
...
def FindMatrix(anglesList): 
    """
    Given input [[],[theta(1,2)],...,[theta(1,N),...,theta(N-1,N)]], output the corresponding matrix T
    """
    N = len(anglesList)
...
def FindHoffmanDecomposition(T): 
    """
    Given a matrix T, output a list of rotations [A_(N(N-1)/2),...,A_1] such that T = A_(N(N-1)/2)x...xA_1 and each A_i only involves two basis elements
    """
    N = T.shape[0]
...
def TestAnglesFunctions(numSamples,N): 
    """
    Generates numSamples elements of SO(N), and checks that FindMatrix(FindAngles(T))=T
    """
...
def TestHoffmanDecomposition(numSamples,N): 
    """
    Generates numSamples elements of SO(N), and checks that the Hoffman decomposition of T multiplies to T
    """
    for i in range(numSamples):
        T = ortho_group.rvs(N)
        T = np.matrix(np.diag([npl.det(T)]+[1]*(N-1)))*T #Converts an element of O(N) to SO(N)
